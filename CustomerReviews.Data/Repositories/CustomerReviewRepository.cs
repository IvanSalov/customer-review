...
namespace CustomerReviews.Data.Repositories
{
    public class CustomerReviewRepository : EFRepositoryBase, ICustomerReviewRepository
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<CustomerReviewAssessmentEntity>().ToTable("CustomerReviewAssessment").HasKey(x => x.Id).Property(x => x.Id);

            base.OnModelCreating(modelBuilder);
        }

        public CustomerReviewEntity GetReviewWithAssessmentsById(string id)
        {
            return CustomerReviews.Include(cr => cr.CustomerReviewAssessments).FirstOrDefault(cr => cr.Id == id);
        }
    }
}
